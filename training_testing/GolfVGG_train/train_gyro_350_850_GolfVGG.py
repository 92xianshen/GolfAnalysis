...
opt = {
       'niter': 100,
       'input_shape': (None, 3, 500),
       'output_num_units': 19,
       'save_epoch': 100,
       'wt_decay': 1e-5,
       'loss_file': 'train_loss_gyro_350_850_GolfVGG.txt',
       'X_train': 'train_test_stan/X_train.npz',
       'y_train': 'train_test_stan/y_train.npz',
       'sensor_selector': np.asarray([5, 6, 7]),
       'sample_selector': np.arange(350, 850),
       'checkpoints_folder': 'checkpoints/',
       'model_name': 'golf_model_gyro_350_850_GolfVGG'
       }
...
params = lasagne.layers.get_all_params(golfVGG.network['l_out'],
                                       trainable=True)
grad = T.grad(loss, params)
updates = lasagne.updates.adam(grad, params)

val_prediction = lasagne.layers.get_output(golfVGG.network['l_out'],
                                           input_var,
                                           deterministic=True)
