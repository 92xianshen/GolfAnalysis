...
opt = {
       ...
       'save_epoch': 100,
       'wt_decay': 1e-5,
       'loss_file': 'train_loss_gyro_350_995_GolfVanillaCNN.txt',
...
params = lasagne.layers.get_all_params(golfVanillaCNN.network['l_out'],
                                       trainable=True)
grad = T.grad(loss, params)
updates = lasagne.updates.adam(grad, params)

val_prediction = lasagne.layers.get_output(golfVanillaCNN.network['l_out'],
                                           input_var,
                                           deterministic=True)
