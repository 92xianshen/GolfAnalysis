...
opt = {
       ...
       'save_epoch': 100,
       'wt_decay': 1e-5,
       'loss_file': 'train_loss_gyro_350_995_GolfResNet.txt',
       ...
       'model_name': 'golf_model_gyro_350_995_GolfResNet'
       }
...
params = lasagne.layers.get_all_params(golfResNet.network['l_out'],
                                       trainable=True)
grad = T.grad(loss, params)
updates = lasagne.updates.adam(grad, params)

val_prediction = lasagne.layers.get_output(golfResNet.network['l_out'],
                                           input_var,
                                           deterministic=True)
