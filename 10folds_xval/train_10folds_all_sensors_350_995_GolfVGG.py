...
#==============================================================================
# 20170601
# hyperparameter selection with 10-folds cross validation using all sensors from 350 to 995 with model GolfVGG
...
# 20170420 sample selection: 350-995
#==============================================================================
...
from GolfVGG import GolfVGG

# global parameters
opt = {
       'log_file': 'train_10folds_xval_log_all_sensors_350_995_GolfVGG.txt',
       'niter': 100,
       'input_shape': (None, 8, 645),
       'output_num_units': 19,
       'save_epoch': 100,
       'val_epoch': 10,
       'wt_decay': 1e-5,
       'loss_file_prefix': 'train_all_sensors_350_995_GolfVGG_loss_fold_',
       'loss_file_suffix': '.txt',
       'val_acc_file_prefix': 'val_acc_all_sensors_350_995_GolfVGG_fold_',
       ...
       'sample_selector': np.arange(350, 995),
       'checkpoints_folder': 'checkpoints/',
       'model_name': 'golf_model_all_sensor_350_995_GolfVGG'
       }
...
#                         W=lasagne.init.GlorotUniform())
    golfVGG = GolfVGG(l_in_shape=opt['input_shape'], 
    ...
    prediction = lasagne.layers.get_output(golfVGG.network['l_out'], input_var)
    loss = lasagne.objectives.categorical_crossentropy(prediction, target_var)
    loss = loss.mean()

    # weight decay
    weightsl2 = lasagne.regularization.regularize_layer_params(golfVGG.network['l_out'],
                                                               lasagne.regularization.l2)
    loss += weightsl2*opt['wt_decay']

    params = lasagne.layers.get_all_params(golfVGG.network['l_out'], trainable=True)
    grad = T.grad(loss, params)
    updates = lasagne.updates.adam(grad, params)

    val_prediction = lasagne.layers.get_output(golfVGG.network['l_out'], input_var,
                                               deterministic=True)
    ...
    for epoch in range(opt['niter']):
        ...
        if (epoch+1)%opt['save_epoch'] == 0:
            np.savez_compressed(opt['checkpoints_folder']+opt['model_name'],
                                *lasagne.layers.get_all_param_values(golfVGG.network['l_out']))
