...
#==============================================================================
# 20170601
# hyperparameter selection with 10-folds cross validation using all sensors from 350 to 995 with model GolfResNet
...
# global parameters
opt = {
       'log_file': 'train_10folds_xval_log_all_sensors_350_995_GolfResNet.txt', 
       'niter': 100,
       ...
       'val_epoch': 10,
       'wt_decay': 1e-5,
       'loss_file_prefix': 'train_all_sensors_350_995_GolfResNet_loss_fold_',
       'loss_file_suffix': '.txt',
       'val_acc_file_prefix': 'val_acc_all_sensors_350_995_GolfResNet_fold_', 
       ...
       'model_name': 'golf_model_all_sensor_350_995_GolfResNet'
       }
...
#                         W=lasagne.init.GlorotUniform())
    ...
    # weight decay
    weightsl2 = lasagne.regularization.regularize_layer_params(golfResNet.network['l_out'], 
                                                               lasagne.regularization.l2)
    loss += weightsl2*opt['wt_decay']
    
    params = lasagne.layers.get_all_params(golfResNet.network['l_out'], trainable=True)
    grad = T.grad(loss, params)
    updates = lasagne.updates.adam(grad, params)
    
    val_prediction = lasagne.layers.get_output(golfResNet.network['l_out'], input_var, 
                                               deterministic=True)
    ...
    for epoch in range(opt['niter']):
        ...
        if (epoch+1)%opt['save_epoch'] == 0:
            np.savez_compressed(opt['checkpoints_folder']+opt['model_name'], 
                                *lasagne.layers.get_all_param_values(golfResNet.network['l_out']))
